...
export function hitTestPillar(point: Vec2, pillar: Pillar, zoom: number): boolean {
  return hitTestRect(pillar, point, HIT_RADIUS_PX / zoom);
}

export function hitTestFabric(point: Vec2, fabric: Fabric, zoom: number): boolean {
  return hitTestRect(fabric, point, HIT_RADIUS_PX / zoom);
}
...
export function hitTestElements(
  ...
  for (const wall of walls) {
    if (hitTestWall(point, wall, zoom)) {
      return { wallId: wall.id, textId: null, pillarId: null, fabricId: null };
    }
  }
  return { wallId: null, textId: null, pillarId: null, fabricId: null };
}
...
export function rectRotateHandleWorldCenter(element: RectLike, zoom: number): Vec2 {
  ...
  const local = {
    x: center.x,
    y: Math.min(element.startY, element.endY) - ROTATE_HANDLE_OFFSET_PX / zoom,
  };
