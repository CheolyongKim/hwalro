...
import { DRAWING_STATUS_STYLES } from '../constants/drawingOptions';
import type { DrawingItem } from '../types/drawing';

interface DrawingListTableProps {
  items: DrawingItem[];
  onDelete: (drawing: DrawingItem) => void;
}
        ...
        <tbody className="divide-y divide-line">
          {items.map((drawing) => (
            <tr key={drawing.id} className="group transition-colors hover:bg-primary-soft/35">
              ...
              <td className="px-5 py-4">
                <span
                  className={`inline-flex rounded-full px-3 py-1.5 text-xs font-bold ${DRAWING_STATUS_STYLES[drawing.status]}`}
                >
                  {drawing.status}
                </span>
              </td>
